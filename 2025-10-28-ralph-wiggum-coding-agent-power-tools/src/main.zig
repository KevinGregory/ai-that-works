...
pub fn main() !void {
    ...
    if (std.mem.eql(u8, command, "fmt")) {
        ...
        if (args.len < 3) {
            try printError("generate command requires a file argument", "minibaml generate <file.baml> [--typescript|--python|--go|--ruby|--rust|--elixir|--typebuilder]");
            return;
        }
        const path = args[2];
        ...
        if (args.len >= 4) {
            if (std.mem.eql(u8, args[3], "--typescript") or std.mem.eql(u8, args[3], "-ts")) {
                ...
                use_rust = true;
            } else if (std.mem.eql(u8, args[3], "--elixir")) {
                use_elixir = true;
        ...
        try generateCommand(allocator, path, use_typescript, use_go, use_ruby, use_rust, use_elixir, typebuilder_only);
...
fn generateCommand(allocator: std.mem.Allocator, path: []const u8, use_typescript: bool, use_go: bool, use_ruby: bool, use_rust: bool, use_elixir: bool, typebuilder_only: bool) !void {
    var buffer = std.ArrayList(u8){};
    defer buffer.deinit(allocator);
